function checkFile(el) {
    ...
    switch (fileFormat) {
        ...
        default:
            ...
            $(el).val("")
            $('#picture1').attr('src', "/images/Default.png")
            $('#mainVideo').attr('src', "/images/Default.png")
            break;
    }
}
...
function addFile() {
    ...
    newInput.val("").attr('input-id', parseInt(newInput.attr("input-id")) + 1).attr('multiple', 'multiple');
...
function newGalleryImage(e) {
    thisNumber = parseInt(e.getAttribute('for').split('[')[1].split(']')[0])
    let input = `<input type="file" class="hide" id="gallerylabel[${thisNumber + 1}]" name="galleryImage" onchange="addGalleryImageHandler(this,${thisNumber + 1})">`;
    $('#contentForm').append(input);
    e.setAttribute('for', `gallerylabel[${thisNumber+1}]`)
}
function addGalleryImageHandler(e,number) {
    var url = URL.createObjectURL(e.files[0]);
    let appending = `<img class="gallery-item" src="~/images/Default.png" title="حذف عکس" onclick="$(this).fadeOut()">`;
    $('.gallery-plus').before(appending);
    let newItem = $('.gallery-item:last');
    newItem[0].outerHTML = `<img class="gallery-item"
                            src="${url}"
                            delete-id="${number}"
...
function deleteGalleryImage(el) {
    $(el).fadeOut();
    let id = $(el).attr('inputId');
    $.ajax({
        url: '/ContentManagement/DeleteGallery',
        method: 'POST',
        data: { "id": id },
    ...
    let num = el.getAttribute('delete-id');
    console.log('NUM========', num)
    $(`[id="gallerylabel[${num}]"]`).remove();
}