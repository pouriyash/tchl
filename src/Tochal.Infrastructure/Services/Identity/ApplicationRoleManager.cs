...
namespace Tochal.Infrastructure.Services.Identity
{
        ...
        IApplicationRoleManager
    {
        ...
        public IList<Role> FindUserRoles(int userId)
        {
            ...
            return userRolesQuery.Include(p=>p.Claims).OrderBy(x => x.Name).ToList();
        }
        ...
        public List<RoleClaim> FindRoleClaimOfCurrentUser()
        {
            var userRoles = FindCurrentUserRoles();
            ...
            var RoleClaimsList = userRoles.Select(p => p.Claims);
        ...
        //{
        //    return _userRoles.Any(p=>p.Role.Name=="ADMIN"&&p.UserId== userId);
        //}
